...
from modules.utils import Doc2VecModel
...
class Preprocessor:
    ...
    def convert_text_to_vectors_doc2vec(
        ...
        assert embeddings_doc2vec is not None, "`embedding_doc2vec` argument must be not None."
        assert isinstance(
            embeddings_doc2vec,
            Doc2Vec), "`embedding_doc2vec` argument must be an instance of Doc2Vec to infer vectors."
        text_vectors_df = self._vectorize_text(
            embeddings_doc2vec, text_column, tokens_list
        )
    ...
    def convert_text_to_vectors_tf_idf(
        ...
        assert text_vectorizer is not None, "`text_vectorizer` argument must be not None."
        assert isinstance(
            text_vectorizer,
            TextVectorization), "`text_vectorizer` argument must be an instance of TextVectorization to infer vectors."
        text_vectors = text_vectorizer(text_column)
    ...
    def convert_text_to_vectors_glove(
        ...
        assert glove_tokenizer is not None, "`glove_tokenizer` argument must be not None."
        assert isinstance(
            glove_tokenizer, Tokenizer), "`glove_tokenizer` argument must be an instance of Tokenizer."
        text_sequences = glove_tokenizer.texts_to_sequences(text_column)
        text_padded = pad_sequences(
            text_sequences,
            maxlen=max_length,
